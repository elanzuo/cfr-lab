...
if __name__ == "__main__":
    ...
    # 场景复现：2人局
    # P0 gets Card 0, P1 gets Card 1
    # P0 Pass, P1 Bet, P0 Bet (Call) -> P1 Wins (Card 1 > 0)
    ...
    print(f"Chance: Deal 0. Hist: {state.history}")

    # Chance: Deal 1 to P1
    state.apply_action(1)
    print(f"Chance: Deal 1. Hist: {state.history}")

    # P0 Pass
    print(f"P0 Info: {state.information_state_string()}")  # Should be "0"
    state.apply_action(Action.PASS)

    # P1 Bet
    print(f"P1 Info: {state.information_state_string()}")  # Should be "1p"
    state.apply_action(Action.BET)

    # P0 Bet (Call)
    print(f"P0 Info: {state.information_state_string()}")  # Should be "0pb"
    state.apply_action(Action.BET)

    print(f"Is Terminal? {state.is_terminal()}")
    print(f"Returns: {state.returns()}")
    # Logic: P0 (Card 0) calls. P1 (Card 1) bets.
    # P1 wins.
    # Pot = 1+1 (Ante) + 1 (P1 Bet) + 1 (P0 Call) = 4.
    # P1 put in 2. P1 Return = 4 - 2 = 2.
    # P0 put in 2. P0 Return = -2.

    assert state.returns() == [-2.0, 2.0]
